...
PmsManager::PmsManager(Stream& stream)
{
  this->_stream = &stream;
}

// Standby mode. For low power consumption and prolong the life of the sensor.
void PmsManager::sleep()
{
...
bool PmsManager::read(DATA& data)
{
  _data = &data;
  loop();
...
bool PmsManager::readUntil(DATA& data, uint16_t timeout)
{
  _data = &data;
...
String PmsManager::sendDisplay(uint16_t particles) {
  ...
  if (particles <= 10){
    ...
    message = "Air Quality: 🟠 Unhealthy for Sensitive Groups";
    ...
    message = "Air Quality: 🟣 Very Unhealthy";
  } else if (particles <= 100) {
    message = "Air Quality: 🟣 Very Unhealthy";
  } else if (particles <= 10000) {
    message = "Air Quality: ⚫ Hazardous";
  }
